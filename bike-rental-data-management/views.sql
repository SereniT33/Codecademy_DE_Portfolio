...
SELECT daily_data.month
    ...
    , SUM(daily_data.customer_trips_trips) AS total_customer_trips
    , ROUND(AVG(daily_data.customer_trips)) AS avg_customer_trips
    , SUM(daily_data.unknown_trips_trips) AS total_unknown_trips
    ...
    , COUNT(daily_data.snow) FILTER (WHERE daily_data.snow=)
